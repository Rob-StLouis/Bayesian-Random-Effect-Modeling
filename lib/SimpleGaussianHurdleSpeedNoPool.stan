...
y_num ~ normal(x_num * beta+ age_num*beta_age+d_num * gamma+ alpha ,sigma);
...
generated quantities {
  ...
  for(i in 1:NN ){
    log_lik[NZ+i] = binomial_logit_lpmf(1|1,theta_NN[i]) + normal_lpdf(y_num[i]|x_num[i,] * beta+ age_num[i]*beta_age+d_num[i,] * gamma+ alpha ,sigma);
  }
