...
y_num ~ normal( ((x_num .* (d_num*beta)) * sum_vec)+ (age_num*beta_age)+(d_num * gamma)+ alpha ,(d_num*sigma));
...
generated quantities {
  ...
  for(i in 1:NZ){
    ...
    if (1==binomial_rng(1,inv_logit(theta_NZ[i]))){
      y_pred[i] = normal_rng( (x_zero[i,] .* (d_zero[i,]*beta))*sum_vec+ (age_zero[i]*beta_age)
  ...
  for(i in 1:NN ){
    log_lik[NZ+i] = binomial_logit_lpmf(1|1,theta_NN[i]) +
                    normal_lpdf(y_num[i]|(x_num[i,] .* (d_num[i,] *beta))*sum_vec + 
                                         (age_num[i]*beta_age)+(d_num[i,] * gamma) + alpha ,
                                         d_num[i,]*sigma);
    
    if (1==binomial_rng(1,inv_logit(theta_NN[i]))){
      y_pred[NZ+i] = normal_rng((x_num[i,] .* (d_num[i,]*beta))*sum_vec + (age_num[i]*beta_age)
      +(d_num[i,] *
